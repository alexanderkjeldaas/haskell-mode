...
;; Copyright (C) 2010, 2013 Chris Done <chrisdone@gmail.com>
...
(defvar haskell-sort-imports-separate-with-empty-line nil
  "Whether an empty line splits imports into sections that should
not be sorted as a group.")
...
(defvar haskell-sort-imports-stop-regexp
  (concat "^[ ]*\\(::"
          ...
          "\\|----*\n"
          ;; Some projects separate import sections using a newline
          (when haskell-sort-imports-separate-with-empty-line
            "\\|.*\n\n")
          "\\|data\\)")
  ...
  (if (use-region-p)
      ...
      (let* ((stop-rx haskell-sort-imports-stop-regexp)
             ...
             (begin (or (save-excursion
                          (when (re-search-backward stop-rx (point-min) t)
                            (match-end 0)))
                        (point-min)))
        ...
        (cond
          ...
          (importp
           (let ((current-line (buffer-substring-no-properties
                                ...
                                (line-end-position)))
                 (col (current-column)))
             (message "Sorting imports around us (b:%d e:%d)" begin end)
          ...
          (t
           ...
           (message "Sorting global import area (b:%d e:%d)" global-begin
                    global-end)
